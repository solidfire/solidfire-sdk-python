...
from solidfire.common import ServiceBase
...
class Element(ServiceBase):
    ...
    def add_account(
        ...
        :param initiator_secret: (optional) CHAP secret to use for the
            ...
            target CHAP secret. 
             If not specified, a random secret
            is created.
    ...
    def list_accounts(
        ...
        :param start_account_id: (optional) Starting AccountID to return. If no
            Account exists with this AccountID, the next Account by AccountID
            order is used as the start of the list. To page through the list,
            pass the AccountID of the last Account in the previous response + 1
        :type start_account_id: int

        :param limit: (optional) Maximum number of AccountInfo objects to
            return.
        :type limit: int
        """
    ...
    def remove_account(
        ...
        removed.
        If volumes on the account are still pending deletion, RemoveAccount
        cannot be used until DeleteVolume to delete and purge the volumes.
    ...
    def get_async_result(
        ...
        response is sent.
        To obtain the result of the method call, polling with GetAsyncResult is
        required.

        GetAsyncResult returns the overall status of the operation (in
    ...
    def create_snapshot(
        ...
        Note: Creating a snapshot is allowed if cluster fullness is at stage 2
        or 3.
        ...
        :param snapshot_id: (optional) Unique ID of a snapshot from which the
            new snapshot is made. The snapshotID passed must be a snapshot on
            the given volume. If a SnapshotID is not provided, a snapshot is
            created from the volume&#39;s active branch.
    ...
    def list_snapshots(
            ...
            volume_id=OPTIONAL,):
        """
        ListSnapshots is used to return the attributes of each snapshot taken
        on the volume.
    ...
    def modify_snapshot(
            ...
            enable_remote_replication=OPTIONAL,):
        """
        ModifySnapshot is used to change the attributes currently assigned to a
        snapshot.
        ...
        :param enable_remote_replication: (optional) Use to enable the snapshot
            ...
            values: true: the snapshot will be replicated to remote storage.
    ...
    def rollback_to_snapshot(
            ...
            attributes=OPTIONAL,):
        """
        RollbackToSnapshot is used to make an existing snapshot the \"active\"
        volume image. This method creates a new
        ...
        unless the parameter saveCurrentState is set with
        a value of \"true.\"
        ...
        :param save_current_state: [required] true: The previous active volume
            image is kept. false: (default) The previous active volume image is
            deleted.
    ...
    def create_group_snapshot(
            ...
            attributes=OPTIONAL,):
        """
        CreateGroupSnapshot is used to create a point-in-time copy of a group
        of volumes.
        The snapshot created can then be used later as a backup or rollback to
        ensure the data
        on the group of volumes is consistent for the point in time in which
        the snapshot was created.

        Note: Creating a group snapshot is allowed if cluster fullness is at
        stage 2 or 3.
        Snapshots are not created when cluster fullness is at stage 4 or 5.
    ...
    def delete_group_snapshot(
        ...
        The saveMembers parameter can be used to preserve all the snapshots
        that
        ...
        :param save_members: [required] true: Snapshots are kept, but group
            association is removed. false: The group and snapshots are deleted.
    ...
    def list_group_snapshots(
            ...
            volume_id=OPTIONAL,):
        """
        ListGroupSnapshots is used to return information about all group
        snapshots that have been created.

        :param volume_id: (optional) An array of unique volume IDs to query. If
            this parameter is not specified, all group snapshots on the cluster
            will be included.
        :type volume_id: int
        """
    ...
    def modify_group_snapshot(
            ...
            enable_remote_replication=OPTIONAL,):
        """
        ModifyGroupSnapshot is used to change the attributes currently assigned
        to a group snapshot.
    ...
    def rollback_to_group_snapshot(
            ...
            attributes=OPTIONAL,):
        """
        RollbackToGroupSnapshot is used to roll back each individual volume in
        a snapshot group to a copy of their individual snapshots.
    ...
    def clone_volume(
        ...
        complete.
        The cloning process begins immediately when the CloneVolume request is
        made and is representative of the state of the volume when the API
        method is issued.
        GetAsyncResults can be used to determine when the cloning process is
        complete and the new volume is available for connections.
        ListSyncJobs can be used to see the progress of creating the clone.

        Note: The initial attributes and quality of service settings for the
        volume are inherited from the volume being cloned.
        ...
        :param new_account_id: (optional) AccountID for the owner of the new
            volume. If unspecified, the AccountID of the owner of the volume
        ...
        :param access: (optional) Access settings for the new volume. readOnly:
            Only read operations are allowed. readWrite: Reads and writes are
            allowed. locked: No reads or writes are allowed. replicationTarget:
            Identify a volume as the target volume for a paired set of volumes.
    ...
    def create_volume(
        ...
        :param qos: (optional) Initial quality of service settings for this
            volume. 
             Volumes created without specified QoS values
            are created with the default values for QoS. Default values for a
            volume can be found by running the GetDefaultQoS method.
        :type qos: QoS
    ...
    def delete_volume(
        ...
        PausedMisconfigured state and data is no longer sent to it or from the
        deleted volume.
    ...
    def get_volume_stats(
            ...
            volume_id,):
        """
        GetVolumeStats is used to retrieve high-level activity measurements for
        a single volume.
    ...
    def list_active_volumes(
            ...
            limit=OPTIONAL,):
        """
        ListActiveVolumes is used to return the list of active volumes
        currently in the system.
        The list of volumes is returned sorted in VolumeID order and can be
    ...
    def list_deleted_volumes(
            self,):
        """
        ListDeletedVolumes is used to return the entire list of volumes that
        have been marked for deletion and is purged from the system.
        """
    ...
    def list_volumes(
            ...
            volume_ids=OPTIONAL,):
        """
        The ListVolumes method is used to return a list of volumes that are in
        a cluster.
        ...
        :param volume_ids: (optional) If specified, only fetch volumes
            ...
            startVolumeID, limit, or accounts option is specified.
        :type volume_ids: int
        """
    ...
    def list_volumes_for_account(
            ...
            limit=OPTIONAL,):
        """
        ListVolumesForAccount returns the list of active AND (pending) deleted
        volumes for an account.
    ...
    def list_volume_stats_by_account(
            self,):
        """
        ListVolumeStatsByAccount returns high-level activity measurements for
        every account.
    ...
    def list_volume_stats_by_volume(
            self,):
        """
        ListVolumeStatsByVolume returns high-level activity measurements for
        every volume, by volume.
        Values are cumulative from the creation of the volume.
        """
    ...
    def list_volume_stats_by_volume_access_group(
        ...
        """
        ListVolumeStatsByVolumeAccessGroup is used to get total activity
        measurements for all of the volumes that are a member of the specified
        volume access group(s).

        :param volume_access_groups: (optional) An array of
            VolumeAccessGroupIDs for which volume activity is returned. If no
            VolumeAccessGroupID is specified, stats for all volume access
            groups is returned.
    ...
    def modify_volume(
        ...
        :param account_id: (optional) AccountID to which the volume is
            reassigned. If none is specified, the previous account name is
            used.
        :type account_id: int

        :param access: (optional) Access allowed for the volume. readOnly: Only
            read operations are allowed. readWrite: Reads and writes are
            allowed. locked: No reads or writes are allowed. replicationTarget:
            Identify a volume as the target volume for a paired set of volumes.
            If the volume is not paired, the access status is locked.
            If unspecified, the access settings of the clone will be the same
    ...
    def purge_deleted_volume(
            ...
            volume_id,):
        """
        PurgeDeletedVolume immediately and permanently purges a volume which
        has been deleted.
    ...
    def restore_deleted_volume(
        ...
        :param volume_id: [required] RestoreDeletedVolume
        :type volume_id: int
        """
    ...
    def list_volume_access_groups(
            ...
            limit=OPTIONAL,):
        """
        ListVolumeAccessGroups is used to return information about the volume
        access groups that are currently in the system.

        :param start_volume_access_group_id: (optional) The lowest
            VolumeAccessGroupID to return. This can be useful for paging. If
            unspecified, there is no lower limit (implicitly 0).
